class BinaryTree:
    ...
    '''префиксный подход. Сначала мы должны обработать значение самого узла (поэтому он и префиксный),
     а затем рекурсивно проделать то же самое с левым потомком и затем с правым.'''
    def pre_order(self):
